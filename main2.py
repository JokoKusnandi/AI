...
# KONFIGURASI UTAMA
# ============================================================
SYMBOLS_TO_TRADE = ["BTCUSD", "XAUUSD"]
TIMEFRAMES = {
    "H4": mt5.TIMEFRAME_H4,
    "H1": mt5.TIMEFRAME_H1,
...
# --- RISK MANAGEMENT ---
BASE_BALANCE = 30          # Modal awal acuan $30
BASE_LOT = 0.01              # Lot dasar untuk modal $30
...
def calculate_dynamic_lot(balance):
    """
    Hitung lot dinamis berdasarkan modal saat ini.
    $30 → 0.01 | $60 → 0.02 | $90 → 0.03 | $120 → 0.04 | dst
    """
...
def execute_trade(symbol, signal, atr_value):
    ...
    request = {
        ...
        "sl": sl,
        "tp": tp,
        "deviation": 20,  # Slippage tolerance dalam points
        "magic": MAGIC_NUMBER,
...
print(f"💡 Lot dinamis aktif: Mulai dari {BASE_LOT} untuk modal ${BASE_BALANCE}")
...
try:
    while not bot_stopped:
        ...
        if not check_daily_loss_limit(current_lot):
            ...
            time.sleep(300)  # Tidur 5 menit sebelum cek lagi
            continue
        ...
        for symbol in SYMBOLS_TO_TRADE:
            # Skip jika sudah ada posisi terbuka di symbol ini
            ...
            if signal != "HOLD" and atr_value is not None and atr_value > 0:
                ...
                print(f"   💰 Lot Dinamis: {current_lot} (Modal: ${current_balance:.2f})")
                ...
                # Status scan (uncomment jika ingin verbose)
                # print(f"⏳ [{datetime.now().strftime('%H:%M:%S')}] {symbol}: HOLD | Lot aktif: {current_lot}")
                pass
