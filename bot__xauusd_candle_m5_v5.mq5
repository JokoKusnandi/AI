...
input double InpTakeProfitPrice = 3.0;    // Jarak Take Profit awal dalam HARGA
...
input double InpSLReductionPerScale = 2.0;// Pengurangan jarak SL dari jarak Initial setiap kali scaling
...
int OnInit()
{
   ...
   Print("   Recovery: Trigger di $", InpRecoveryLossTrigger, " | Basket Close di $", InpBasketCloseProfit);
...
void ManageScalingEntry()
{
   ...
   if(initialProfit >= requiredProfit) {
      ...
      // PERBAIKAN UTAMA: SYMBOL_TRADE_STOPS_LEVEL adalah INTEGER, bukan DOUBLE
         ...
         if(StringFind(comment, "Initial") >= 0) {
            ...
            if(profit <= InpRecoveryLossTrigger) {
               if(type == POSITION_TYPE_BUY) {
                  ...
                  OpenCustomSellOrder(InpHedgeLotSize, "Sell - HEDGE 1", 0.0, 0.0); 
                  OpenCustomSellOrder(InpHedgeLotSize, "Sell - HEDGE 2", 0.0, 0.0);
               }
               else if(type == POSITION_TYPE_SELL) {
                  Print("⚠️ RECOVERY TRIGGERED! Sell loss: $", profit, ". Opening 2x BUY HEDGE");
                  OpenCustomBuyOrder(InpHedgeLotSize, "Buy - HEDGE 1", 0.0, 0.0);
                  OpenCustomBuyOrder(InpHedgeLotSize, "Buy - HEDGE 2", 0.0, 0.0);
               }
            }
         }
      }
