...
input double InpTakeProfitPrice = 12.0;    // Jarak Take Profit awal dalam HARGA
...
input double InpScalingTrigger = 2.0;     // Tambah posisi setiap profit awal kelipatan nilai ini (USD)
input double InpScalingLot = 0.01;        // Lot size untuk posisi tambahan (Scaling)
input double InpTPReductionPerScale = 3.0;// Pengurangan jarak TP dari jarak Initial setiap kali scaling
input double InpSLReductionPerScale = 7.0;// Pengurangan jarak SL dari jarak Initial setiap kali scaling
...
input double InpHedgeStopLossPrice = 10.0;   // Jarak Stop Loss khusus untuk posisi HEDGE (dalam HARGA)
input double InpHedgeTakeProfitPrice = 5.0; // Jarak Take Profit khusus untuk posisi HEDGE (dalam HARGA)
input double InpBasketCloseProfit = 0.50;   // TUTUP SEMUA posisi jika total profit gabungan mencapai nilai ini

// FITUR RECOVERY DARI POSISI SCALE
input double InpScaleRecoveryTrigger = -10.0; // Segera buka Initial lawan jika profit posisi Scale <= nilai ini (USD)
...
// GLOBAL VARIABLES
// ====================================================================
string g_RecoveredScaleTickets = ""; // Menyimpan ticket posisi scale yang sudah di-recovery agar tidak spam
...
int OnInit()
{
   ...
   Print("   Recovery Hedge: Trigger di $", InpRecoveryLossTrigger, " | Hedge SL: ", InpHedgeStopLossPrice, " | Hedge TP: ", InpHedgeTakeProfitPrice);
   Print("   Scale Recovery: Trigger di $", InpScaleRecoveryTrigger);
...
void OnTick()
{
   ...
   if(posCount > 0)
   {
      ...
      if(posCount >= 3)
      {
         ...
         ManageScaleRecovery(); // Tetap cek recovery scale meski dalam mode hedge
         return; 
      }
      ...
      ManageBreakEven();
      ManageScaleRecovery(); // Cek jika posisi scale rugi, buka initial lawan
...
//+------------------------------------------------------------------+
//| FUNGSI RECOVERY DARI POSISI SCALE YANG RUGI                      |
//+------------------------------------------------------------------+
void ManageScaleRecovery()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if(ticket > 0 && PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == InpMagicNumber) {
         ...
         // Hanya proses posisi "Scale" yang bukan posisi "Recovery"
         if(StringFind(comment, "Scale") >= 0 && StringFind(comment, "Recovery") < 0) {
            
            // Cek apakah ticket ini sudah pernah memicu recovery (mencegah spam order)
            if(StringFind(g_RecoveredScaleTickets, IntegerToString(ticket) + ",") >= 0) {
               continue; 
            }
            
            // Jika SELL Scale rugi <= trigger, buka BUY Initial Recovery
            if(type == POSITION_TYPE_SELL && profit <= InpScaleRecoveryTrigger) {
               Print("⚠️ SELL Scale loss reached ", InpScaleRecoveryTrigger, "! Opening BUY Initial Recovery.");
               // PERBAIKAN: Fungsi sekarang mengembalikan bool, sehingga valid digunakan di dalam 'if'
               if(OpenCustomBuyOrder(InpLotSize, "Buy - Scale Recovery", 0.0, 0.0)) {
                  g_RecoveredScaleTickets += IntegerToString(ticket) + ",";
               }
            }
            // Jika BUY Scale rugi <= trigger, buka SELL Initial Recovery
            else if(type == POSITION_TYPE_BUY && profit <= InpScaleRecoveryTrigger) {
               Print("⚠️ BUY Scale loss reached ", InpScaleRecoveryTrigger, "! Opening SELL Initial Recovery.");
               // PERBAIKAN: Fungsi sekarang mengembalikan bool, sehingga valid digunakan di dalam 'if'
               if(OpenCustomSellOrder(InpLotSize, "Sell - Scale Recovery", 0.0, 0.0)) {
                  g_RecoveredScaleTickets += IntegerToString(ticket) + ",";
               }
            }
         }
...
// PERBAIKAN: Mengubah return type dari 'void' menjadi 'bool'
bool OpenCustomBuyOrder(double lotSize, string customComment, double forcedSL, double forcedTP)
{
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   ...
   bool result = trade.Buy(lotSize, _Symbol, ask, sl, tp, customComment);
   if(result) {
...
// PERBAIKAN: Mengubah return type dari 'void' menjadi 'bool'
bool OpenCustomSellOrder(double lotSize, string customComment, double forcedSL, double forcedTP)
{
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   ...
   bool result = trade.Sell(lotSize, _Symbol, bid, sl, tp, customComment);
   if(result) {
